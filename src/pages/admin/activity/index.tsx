...
import UpdateActivity from "./popups/update";
import CreateActivity from "./popups/create";
...
const Activities = () => {
  ...
  useEffect(() => {
    setBreadcrumb([
      { text: messages["Admin"], url: `${AppConsts.publicUrl}/admin` },
      {
        ...
        url: `${AppConsts.publicUrl}/admin/activities`,
      },
    ]);
  ...
  const deleteConfig: any = (id: any) => {
    return {
      ...
      title: messages["Confirmation.Delete.Activity"],
      ...
      async onOk() {
        if (!MainUtils.isEmptyValue(id)) {
          ...
          if (res) {
            setLoader(true);
            const dataActivity: any = await Service.getAll();
